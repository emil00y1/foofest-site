...
function NextBtn({ setErrorMsg, setPageView, pageView, amount, chosenArea, acceptedTerms, setTermsError, paymentData, setEmailError }) {
  ...
  const [containsValidPersonalEmail, setContainsValidPersonalEmail] = useState(true);

  // Update containsEmptyString whenever amount changes
  useEffect(() => {
    setContainsEmptyString(amount.some((obj) => Object.values(obj).some((value) => value === "")));
  }, [amount]);
  ...
  useEffect(() => {
    setContainsEmptyString2(paymentData.some((obj) => Object.values(obj).some((value) => value === "")));
